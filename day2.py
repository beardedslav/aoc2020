...
def passwordIsValid(passwordPolicy):
    occurences, letter, password = passwordPolicy.split(" ")
    minOccurences, maxOccurences = occurences.split("-")
    letter = letter[0]
    actualOccurences = Counter(password).get(letter, 0)
    if actualOccurences >= int(minOccurences) and actualOccurences <= int(maxOccurences):
        return 1
    else:
        return 0


def passwordIsValidNewRule(passwordPolicy):
    positions, letter, password = passwordPolicy.split(" ")
    position1, position2 = positions.split("-")
    letter = letter[0]
    return int(( password[int(position1)-1] == letter  ) != ( password[int(position2)-1] == letter  )  )


input = input.split("\n")
print(sum( passwordIsValid(inputLine) for inputLine in input ))
print(sum( passwordIsValidNewRule(inputLine) for inputLine in input ))
